...
class Trader:
    ...
    def __init__(self):
        ...
        self.tomato_bought = False
        self.tomato_sold = False
    ...
    def trade_tomato(self, state):
        ...
        tomato_trades = state.market_trades.get('TOMATOES')

        if tomato_trades is not None and len(tomato_trades) > 0:
            vwap = self.VWAP(tomato_trades)
            if vwap is not None:
                self.tomato_prices.append(vwap)


        if len(self.tomato_prices) > 15:
            self.tomato_prices.pop(0)
        ...
        if len(sell_orders) != 0 and len(buy_orders) != 0:
            ...
            # Take positions more than one STD away from the mean, in the opposite direction
            if len(self.tomato_prices) >= 15:
                tomatoSD = statistics.stdev(self.tomato_prices)
                tomatoMean = statistics.mean(self.tomato_prices)
                print("TOMATO MEAN: " + str(tomatoMean))
                print("TOMATO SD: " + str(tomatoSD))
                self.search_buys(state, 'TOMATOES', tomatoMean - 1.2*tomatoSD, depth=3)
                self.search_sells(state, 'TOMATOES', tomatoMean + 1.2*tomatoSD, depth=3)

            logger.print(f"TOMATOES FAIR PRICE: {decimal_fair_price}")
            if not self.tomato_sold:
                self.search_buys(state, 'TOMATOES', decimal_fair_price, depth=3)
            if not self.tomato_bought:
